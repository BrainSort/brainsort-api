...
export class DiagnosticsService {
  ...
  async evaluar(usuarioId: string, respuestasUsuario: number[]) {
    ...
    const bubble = algoritmos.find(a => a.nombre.toLowerCase().includes('bubble'))?.id;
    const selection = algoritmos.find(a => a.nombre.toLowerCase().includes('selection'))?.id;
    const insertion = algoritmos.find(a => a.nombre.toLowerCase().includes('insertion'))?.id;
