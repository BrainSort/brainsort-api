...
export const BinarySearch: AlgorithmDefinition = {
  ...
  execute(data: number[]): SimulationStep[] {
    ...
    const getMarkers = (l: number, h: number, m: number, targetFoundIndex?: number): SimulationMarker[] => {
      const markersList: SimulationMarker[] = [];
      ...
      for (let i = 0; i < array.length; i++) {
        if (indexLabels[i] === undefined && (i < l || i > h)) {
          markersList.push({
            ...
            label: '',
            role: 'descartado',
            color: '#1A2333', // Faded blue-gray for discarded elements
          });
        }
      }
    ...
    while (low <= high) {
      ...
      lastMid = mid;
      steps.push(createStep(step++, 'comparacion', [mid], array, 3, [], getMarkers(low, high, mid)));
      if (array[mid] === target) {
        steps.push(createStep(step++, 'final', [mid], array, 4, [], getMarkers(low, high, mid, mid)));
        return steps;
      }
      if (array[mid] < target) low = mid + 1;
      else high = mid - 1;
      steps.push(
        createStep(step++, 'insercion', [], array, array[mid] < target ? 5 : 6, [], getMarkers(low, high, mid)),
      );
    }
    steps.push(createStep(step++, 'final', [], array, 6, [], getMarkers(low, high, lastMid)));
    return steps;
  },
};
